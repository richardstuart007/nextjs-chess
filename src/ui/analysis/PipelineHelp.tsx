...
const STEPS = [
  ...
  },
  {
    ...
    processing:
      'For each position that has a Stockfish evaluation but no insight yet, sends the FEN, CP score, and move history to Claude AI. Claude returns a short coaching theme and practical improvement advice. Runs in batches of 20; repeat until processed = 0.',
    output: [
...
const ROW_COUNT_SQL =
  ...
  UNION ALL SELECT 5, 'tmov_moves',             COUNT(*) FROM tmov_moves
  UNION ALL SELECT 6, 'tgam_game_positions',    COUNT(*) FROM tgam_game_positions
  UNION ALL SELECT 7, 'teva_evaluations',       COUNT(*) FROM teva_evaluations
  UNION ALL SELECT 8, 'tins_insights',          COUNT(*) FROM tins_insights
) t ORDER BY ord;`
